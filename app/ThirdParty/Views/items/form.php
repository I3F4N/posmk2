...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var cost_percentage = <?= isset($config['cost_price_percentage']) && $config['cost_price_percentage'] !== '' ? (float)$config['cost_price_percentage'] : 55 ?>;
        $('#unit_price').on('input', function() {
            var unit_price = parseFloat($(this).val());
            if(!isNaN(unit_price)) {
                var cost = unit_price * (cost_percentage / 100);
                $('#cost_price').val(cost.toFixed(2));
            }
        });
